...
> Object createClusterCustomObject(group, version, plural, body).pretty(pretty).dryRun(dryRun).fieldManager(fieldManager).fieldValidation(fieldValidation).execute();
...
> Object createNamespacedCustomObject(group, version, namespace, plural, body).pretty(pretty).dryRun(dryRun).fieldManager(fieldManager).fieldValidation(fieldValidation).execute();
...
> Object deleteCollectionClusterCustomObject(group, version, plural).pretty(pretty).labelSelector(labelSelector).gracePeriodSeconds(gracePeriodSeconds).orphanDependents(orphanDependents).propagationPolicy(propagationPolicy).dryRun(dryRun).body(body).execute();
...
> Object patchClusterCustomObject(group, version, plural, name, body).dryRun(dryRun).fieldManager(fieldManager).fieldValidation(fieldValidation).force(force).execute();
...
> Object patchClusterCustomObjectScale(group, version, plural, name, body).dryRun(dryRun).fieldManager(fieldManager).fieldValidation(fieldValidation).force(force).execute();
...
> Object patchClusterCustomObjectStatus(group, version, plural, name, body).dryRun(dryRun).fieldManager(fieldManager).fieldValidation(fieldValidation).force(force).execute();
...
> Object patchNamespacedCustomObject(group, version, namespace, plural, name, body).dryRun(dryRun).fieldManager(fieldManager).fieldValidation(fieldValidation).force(force).execute();
...
> Object patchNamespacedCustomObjectScale(group, version, namespace, plural, name, body).dryRun(dryRun).fieldManager(fieldManager).fieldValidation(fieldValidation).force(force).execute();
...
> Object patchNamespacedCustomObjectStatus(group, version, namespace, plural, name, body).dryRun(dryRun).fieldManager(fieldManager).fieldValidation(fieldValidation).force(force).execute();
...
> Object replaceClusterCustomObject(group, version, plural, name, body).dryRun(dryRun).fieldManager(fieldManager).fieldValidation(fieldValidation).execute();
...
> Object replaceClusterCustomObjectScale(group, version, plural, name, body).dryRun(dryRun).fieldManager(fieldManager).fieldValidation(fieldValidation).execute();
...
> Object replaceClusterCustomObjectStatus(group, version, plural, name, body).dryRun(dryRun).fieldManager(fieldManager).fieldValidation(fieldValidation).execute();
...
> Object replaceNamespacedCustomObject(group, version, namespace, plural, name, body).dryRun(dryRun).fieldManager(fieldManager).fieldValidation(fieldValidation).execute();
...
> Object replaceNamespacedCustomObjectScale(group, version, namespace, plural, name, body).dryRun(dryRun).fieldManager(fieldManager).fieldValidation(fieldValidation).execute();
...
> Object replaceNamespacedCustomObjectStatus(group, version, namespace, plural, name, body).dryRun(dryRun).fieldManager(fieldManager).fieldValidation(fieldValidation).execute();
