...
public class ExtensionsV1beta1RollingUpdateDeployment {
   ...
   * MaxUnavailable is 0. Absolute number is calculated from percentage by rounding up. By default,
   * a value of 1 is used. Example: when this is set to 30%, the new RC can be scaled up immediately
   * when the rolling update starts, such that the total number of old and new pods do not exceed
   * 130% of desired pods. Once old pods have been killed, new RC can be scaled up further, ensuring
   * that total number of pods running at any time during the update is atmost 130% of desired pods.
   *
   * @return maxSurge
   */
  @ApiModelProperty(
   ...
   * percentage by rounding down. This can not be 0 if MaxSurge is 0. By default, a fixed value of 1
   * is used. Example: when this is set to 30%, the old RC can be scaled down to 70% of desired pods
   * immediately when the rolling update starts. Once new pods are ready, old RC can be scaled down
   * further, followed by scaling up the new RC, ensuring that the total number of pods available at
