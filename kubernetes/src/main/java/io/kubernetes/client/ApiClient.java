    ...
    try {
      ...
      if (keyManagers != null || trustManagers != null) {
        ...
        if (trustManager != null) {
          setSslSocketFactory(httpClient, sslContext.getSocketFactory(), trustManager);
        } else {
          setSslSocketFactory(httpClient, sslContext.getSocketFactory());
        }

      } else {
        setHttpClient(setSslSocketFactory(httpClient, null, null));
      }
      setHttpClient(setHostnameVerifier(httpClient, hostnameVerifier));
    } catch (GeneralSecurityException e) {
      throw new RuntimeException(e);
    }
  }

  private static OkHttpClient setSslSocketFactory(
      OkHttpClient client, SSLSocketFactory factory, X509TrustManager trustManager) {
    if (factory == null) return client;
    if (trustManager == null) return client;
    return client.newBuilder().sslSocketFactory(factory, trustManager).build();
  }

  private static OkHttpClient setSslSocketFactory(OkHttpClient client, SSLSocketFactory factory) {
    if (factory == null) return client;
    return client.newBuilder().sslSocketFactory(factory).build();
  }

  private static OkHttpClient setHostnameVerifier(OkHttpClient client, HostnameVerifier verifier) {
    if (verifier == null) return client;
    return client.newBuilder().hostnameVerifier(verifier).build();
  }
