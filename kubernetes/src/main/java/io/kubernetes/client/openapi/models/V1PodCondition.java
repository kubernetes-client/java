...
public class V1PodCondition {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("lastProbeTime", "lastTransitionTime", "message", "observedGeneration", "reason", "status", "type"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!V1PodCondition.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in V1PodCondition is not found in the empty JSON string", V1PodCondition.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!V1PodCondition.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `V1PodCondition` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
