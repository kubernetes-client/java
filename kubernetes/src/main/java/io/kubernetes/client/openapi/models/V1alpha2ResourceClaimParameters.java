...
public class V1alpha2ResourceClaimParameters implements io.kubernetes.client.common.KubernetesObject {
  ...
  private List<V1alpha2DriverRequests> driverRequests = new ArrayList<>();
  ...
  * @throws IOException if the JSON Element is invalid with respect to V1alpha2ResourceClaimParameters
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!V1alpha2ResourceClaimParameters.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!V1alpha2ResourceClaimParameters.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `V1alpha2ResourceClaimParameters` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("driverRequests") != null && !jsonObj.get("driverRequests").isJsonNull()) {
        ...
        if (jsonArraydriverRequests != null) {
          ...
          for (int i = 0; i < jsonArraydriverRequests.size(); i++) {
            V1alpha2DriverRequests.validateJsonElement(jsonArraydriverRequests.get(i));
          };
        }
      }
      // validate the optional field `generatedFrom`
      if (jsonObj.get("generatedFrom") != null && !jsonObj.get("generatedFrom").isJsonNull()) {
        V1alpha2ResourceClaimParametersReference.validateJsonElement(jsonObj.get("generatedFrom"));
      }
