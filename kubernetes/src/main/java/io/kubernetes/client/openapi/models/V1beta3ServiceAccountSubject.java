...
public class V1beta3ServiceAccountSubject {
  ...
  * @throws IOException if the JSON Element is invalid with respect to V1beta3ServiceAccountSubject
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!V1beta3ServiceAccountSubject.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!V1beta3ServiceAccountSubject.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `V1beta3ServiceAccountSubject` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
