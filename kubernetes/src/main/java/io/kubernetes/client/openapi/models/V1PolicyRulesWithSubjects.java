...
public class V1PolicyRulesWithSubjects {
  ...
  * @throws IOException if the JSON Object is invalid with respect to V1PolicyRulesWithSubjects
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!V1PolicyRulesWithSubjects.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!V1PolicyRulesWithSubjects.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `V1PolicyRulesWithSubjects` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      if (jsonObj.get("nonResourceRules") != null && !jsonObj.get("nonResourceRules").isJsonNull()) {
        ...
        if (jsonArraynonResourceRules != null) {
          ...
          for (int i = 0; i < jsonArraynonResourceRules.size(); i++) {
            V1NonResourcePolicyRule.validateJsonObject(jsonArraynonResourceRules.get(i).getAsJsonObject());
          };
        }
      }
      if (jsonObj.get("resourceRules") != null && !jsonObj.get("resourceRules").isJsonNull()) {
        ...
        if (jsonArrayresourceRules != null) {
          ...
          for (int i = 0; i < jsonArrayresourceRules.size(); i++) {
            V1ResourcePolicyRule.validateJsonObject(jsonArrayresourceRules.get(i).getAsJsonObject());
          };
        }
      }
      // ensure the json data is an array
      ...
      for (int i = 0; i < jsonArraysubjects.size(); i++) {
        FlowcontrolV1Subject.validateJsonObject(jsonArraysubjects.get(i).getAsJsonObject());
      };
  }
