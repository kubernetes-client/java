...
public class V1beta3PriorityLevelConfigurationStatus {
  ...
  private List<V1beta3PriorityLevelConfigurationCondition> conditions = new ArrayList<>();
  ...
  * @throws IOException if the JSON Element is invalid with respect to V1beta3PriorityLevelConfigurationStatus
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!V1beta3PriorityLevelConfigurationStatus.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!V1beta3PriorityLevelConfigurationStatus.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `V1beta3PriorityLevelConfigurationStatus` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("conditions") != null && !jsonObj.get("conditions").isJsonNull()) {
        ...
        if (jsonArrayconditions != null) {
          ...
          for (int i = 0; i < jsonArrayconditions.size(); i++) {
            V1beta3PriorityLevelConfigurationCondition.validateJsonElement(jsonArrayconditions.get(i));
          };
        }
      }
  }
