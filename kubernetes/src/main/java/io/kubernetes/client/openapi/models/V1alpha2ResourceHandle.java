...
public class V1alpha2ResourceHandle {
  ...
  * @throws IOException if the JSON Element is invalid with respect to V1alpha2ResourceHandle
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!V1alpha2ResourceHandle.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!V1alpha2ResourceHandle.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `V1alpha2ResourceHandle` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("structuredData") != null && !jsonObj.get("structuredData").isJsonNull()) {
        V1alpha2StructuredResourceHandle.validateJsonElement(jsonObj.get("structuredData"));
      }
  }
