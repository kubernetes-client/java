...
public class V1beta1ResourceSliceSpec {
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!V1beta1ResourceSliceSpec.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in V1beta1ResourceSliceSpec is not found in the empty JSON string", V1beta1ResourceSliceSpec.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!V1beta1ResourceSliceSpec.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `V1beta1ResourceSliceSpec` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : V1beta1ResourceSliceSpec.openapiRequiredFields) {
      ...
      if (jsonObj.get("devices") != null && !jsonObj.get("devices").isJsonNull()) {
        ...
        if (jsonArraydevices != null) {
          ...
          for (int i = 0; i < jsonArraydevices.size(); i++) {
            V1beta1Device.validateJsonElement(jsonArraydevices.get(i));
          };
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!V1beta1ResourceSliceSpec.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'V1beta1ResourceSliceSpec' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<V1beta1ResourceSliceSpec> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(V1beta1ResourceSliceSpec.class));

       return (TypeAdapter<T>) new TypeAdapter<V1beta1ResourceSliceSpec>() {
           @Override
           public void write(JsonWriter out, V1beta1ResourceSliceSpec value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public V1beta1ResourceSliceSpec read(JsonReader in) throws IOException {
