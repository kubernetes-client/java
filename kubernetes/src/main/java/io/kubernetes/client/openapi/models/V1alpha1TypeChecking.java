...
public class V1alpha1TypeChecking {
  ...
  private List<V1alpha1ExpressionWarning> expressionWarnings = new ArrayList<>();
  ...
  * @throws IOException if the JSON Element is invalid with respect to V1alpha1TypeChecking
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!V1alpha1TypeChecking.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!V1alpha1TypeChecking.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `V1alpha1TypeChecking` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("expressionWarnings") != null && !jsonObj.get("expressionWarnings").isJsonNull()) {
        ...
        if (jsonArrayexpressionWarnings != null) {
          ...
          for (int i = 0; i < jsonArrayexpressionWarnings.size(); i++) {
            V1alpha1ExpressionWarning.validateJsonElement(jsonArrayexpressionWarnings.get(i));
          };
        }
      }
  }
