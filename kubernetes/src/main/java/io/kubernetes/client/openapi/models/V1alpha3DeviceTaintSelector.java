...
public class V1alpha3DeviceTaintSelector {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("device", "deviceClassName", "driver", "pool", "selectors"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!V1alpha3DeviceTaintSelector.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in V1alpha3DeviceTaintSelector is not found in the empty JSON string", V1alpha3DeviceTaintSelector.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!V1alpha3DeviceTaintSelector.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `V1alpha3DeviceTaintSelector` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
