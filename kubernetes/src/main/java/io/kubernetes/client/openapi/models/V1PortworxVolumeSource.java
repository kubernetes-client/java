...
public class V1PortworxVolumeSource {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("fsType", "readOnly", "volumeID"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!V1PortworxVolumeSource.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in V1PortworxVolumeSource is not found in the empty JSON string", V1PortworxVolumeSource.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!V1PortworxVolumeSource.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `V1PortworxVolumeSource` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
