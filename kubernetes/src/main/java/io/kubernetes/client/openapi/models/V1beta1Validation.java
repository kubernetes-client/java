...
public class V1beta1Validation {
  ...
  * @throws IOException if the JSON Object is invalid with respect to V1beta1Validation
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!V1beta1Validation.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!V1beta1Validation.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `V1beta1Validation` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
