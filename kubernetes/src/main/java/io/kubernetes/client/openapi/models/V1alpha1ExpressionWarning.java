...
public class V1alpha1ExpressionWarning {
  ...
  * @throws IOException if the JSON Object is invalid with respect to V1alpha1ExpressionWarning
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!V1alpha1ExpressionWarning.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!V1alpha1ExpressionWarning.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `V1alpha1ExpressionWarning` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       ...
       final TypeAdapter<V1alpha1ExpressionWarning> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(V1alpha1ExpressionWarning.class));

       return (TypeAdapter<T>) new TypeAdapter<V1alpha1ExpressionWarning>() {
           @Override
