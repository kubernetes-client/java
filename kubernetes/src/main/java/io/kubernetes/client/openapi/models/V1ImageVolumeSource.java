...
public class V1ImageVolumeSource {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("pullPolicy", "reference"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!V1ImageVolumeSource.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in V1ImageVolumeSource is not found in the empty JSON string", V1ImageVolumeSource.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!V1ImageVolumeSource.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `V1ImageVolumeSource` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      if ((jsonObj.get("pullPolicy") != null && !jsonObj.get("pullPolicy").isJsonNull()) && !jsonObj.get("pullPolicy").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "Expected the field `pullPolicy` to be a primitive type in the JSON string but got `%s`", jsonObj.get("pullPolicy").toString()));
      }
