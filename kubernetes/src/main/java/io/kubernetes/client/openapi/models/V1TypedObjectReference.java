...
public class V1TypedObjectReference {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("apiGroup", "kind", "name", "namespace"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!V1TypedObjectReference.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in V1TypedObjectReference is not found in the empty JSON string", V1TypedObjectReference.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!V1TypedObjectReference.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `V1TypedObjectReference` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
