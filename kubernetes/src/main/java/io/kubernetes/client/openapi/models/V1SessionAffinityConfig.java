...
public class V1SessionAffinityConfig {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("clientIP"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!V1SessionAffinityConfig.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in V1SessionAffinityConfig is not found in the empty JSON string", V1SessionAffinityConfig.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!V1SessionAffinityConfig.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `V1SessionAffinityConfig` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      // validate the optional field `clientIP`
      if (jsonObj.get("clientIP") != null && !jsonObj.get("clientIP").isJsonNull()) {
        V1ClientIPConfig.validateJsonElement(jsonObj.get("clientIP"));
      }
  }
