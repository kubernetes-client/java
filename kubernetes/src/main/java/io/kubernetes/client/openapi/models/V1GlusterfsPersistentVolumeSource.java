...
public class V1GlusterfsPersistentVolumeSource {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("endpoints", "endpointsNamespace", "path", "readOnly"));

    // a set of required properties/fields (JSON key names)
    openapiRequiredFields = new HashSet<String>(Arrays.asList("endpoints", "path"));
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!V1GlusterfsPersistentVolumeSource.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in V1GlusterfsPersistentVolumeSource is not found in the empty JSON string", V1GlusterfsPersistentVolumeSource.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!V1GlusterfsPersistentVolumeSource.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `V1GlusterfsPersistentVolumeSource` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (!jsonObj.get("endpoints").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "Expected the field `endpoints` to be a primitive type in the JSON string but got `%s`", jsonObj.get("endpoints").toString()));
      }
      if ((jsonObj.get("endpointsNamespace") != null && !jsonObj.get("endpointsNamespace").isJsonNull()) && !jsonObj.get("endpointsNamespace").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "Expected the field `endpointsNamespace` to be a primitive type in the JSON string but got `%s`", jsonObj.get("endpointsNamespace").toString()));
      }
