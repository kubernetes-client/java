...
public class V2ExternalMetricSource {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("metric", "target"));

    // a set of required properties/fields (JSON key names)
    openapiRequiredFields = new HashSet<String>(Arrays.asList("metric", "target"));
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!V2ExternalMetricSource.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in V2ExternalMetricSource is not found in the empty JSON string", V2ExternalMetricSource.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!V2ExternalMetricSource.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `V2ExternalMetricSource` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
