...
public class V1ScaleSpec {
  ...
  public V1ScaleSpec replicas(@jakarta.annotation.Nullable Integer replicas) {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("replicas"));
   ...
   * @throws IOException if the JSON Element is invalid with respect to V1ScaleSpec
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!V1ScaleSpec.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in V1ScaleSpec is not found in the empty JSON string", V1ScaleSpec.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!V1ScaleSpec.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `V1ScaleSpec` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
