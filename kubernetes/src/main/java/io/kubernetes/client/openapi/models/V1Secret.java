...
public class V1Secret implements io.kubernetes.client.common.KubernetesObject {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("apiVersion", "data", "immutable", "kind", "metadata", "stringData", "type"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!V1Secret.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in V1Secret is not found in the empty JSON string", V1Secret.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!V1Secret.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `V1Secret` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
