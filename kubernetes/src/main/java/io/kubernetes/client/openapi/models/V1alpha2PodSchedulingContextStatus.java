...
public class V1alpha2PodSchedulingContextStatus {
  ...
  private List<V1alpha2ResourceClaimSchedulingStatus> resourceClaims = new ArrayList<>();
  ...
  * @throws IOException if the JSON Element is invalid with respect to V1alpha2PodSchedulingContextStatus
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!V1alpha2PodSchedulingContextStatus.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!V1alpha2PodSchedulingContextStatus.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `V1alpha2PodSchedulingContextStatus` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("resourceClaims") != null && !jsonObj.get("resourceClaims").isJsonNull()) {
        ...
        if (jsonArrayresourceClaims != null) {
          ...
          for (int i = 0; i < jsonArrayresourceClaims.size(); i++) {
            V1alpha2ResourceClaimSchedulingStatus.validateJsonElement(jsonArrayresourceClaims.get(i));
          };
        }
      }
  }
