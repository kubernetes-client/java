...
public class V1TypeChecking {
  ...
  private List<V1ExpressionWarning> expressionWarnings;
  ...
  * @throws IOException if the JSON Object is invalid with respect to V1TypeChecking
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!V1TypeChecking.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!V1TypeChecking.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `V1TypeChecking` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      if (jsonObj.get("expressionWarnings") != null && !jsonObj.get("expressionWarnings").isJsonNull()) {
        ...
        if (jsonArrayexpressionWarnings != null) {
          ...
          for (int i = 0; i < jsonArrayexpressionWarnings.size(); i++) {
            V1ExpressionWarning.validateJsonObject(jsonArrayexpressionWarnings.get(i).getAsJsonObject());
          };
        }
      }
  }
