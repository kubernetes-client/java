...
public class V1PriorityLevelConfigurationStatus {
  ...
  private List<V1PriorityLevelConfigurationCondition> conditions;
  ...
  * @throws IOException if the JSON Object is invalid with respect to V1PriorityLevelConfigurationStatus
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!V1PriorityLevelConfigurationStatus.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!V1PriorityLevelConfigurationStatus.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `V1PriorityLevelConfigurationStatus` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      if (jsonObj.get("conditions") != null && !jsonObj.get("conditions").isJsonNull()) {
        ...
        if (jsonArrayconditions != null) {
          ...
          for (int i = 0; i < jsonArrayconditions.size(); i++) {
            V1PriorityLevelConfigurationCondition.validateJsonObject(jsonArrayconditions.get(i).getAsJsonObject());
          };
        }
      }
  }
