...
public class V1CSIStorageCapacityList implements io.kubernetes.client.common.KubernetesListObject {
  ...
  * @throws IOException if the JSON Object is invalid with respect to V1CSIStorageCapacityList
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!V1CSIStorageCapacityList.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!V1CSIStorageCapacityList.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `V1CSIStorageCapacityList` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArrayitems.size(); i++) {
        V1CSIStorageCapacity.validateJsonObject(jsonArrayitems.get(i).getAsJsonObject());
      };
