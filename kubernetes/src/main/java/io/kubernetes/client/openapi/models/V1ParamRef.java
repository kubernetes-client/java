...
public class V1ParamRef {
  ...
  public boolean equals(Object o) {
    ...
    return Objects.equals(this.name, v1ParamRef.name) &&
        Objects.equals(this.namespace, v1ParamRef.namespace) &&
        Objects.equals(this.parameterNotFoundAction, v1ParamRef.parameterNotFoundAction) &&
        Objects.equals(this.selector, v1ParamRef.selector);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!V1ParamRef.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in V1ParamRef is not found in the empty JSON string", V1ParamRef.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!V1ParamRef.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `V1ParamRef` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if ((jsonObj.get("parameterNotFoundAction") != null && !jsonObj.get("parameterNotFoundAction").isJsonNull()) && !jsonObj.get("parameterNotFoundAction").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `parameterNotFoundAction` to be a primitive type in the JSON string but got `%s`", jsonObj.get("parameterNotFoundAction").toString()));
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!V1ParamRef.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'V1ParamRef' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<V1ParamRef> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(V1ParamRef.class));

       return (TypeAdapter<T>) new TypeAdapter<V1ParamRef>() {
           @Override
