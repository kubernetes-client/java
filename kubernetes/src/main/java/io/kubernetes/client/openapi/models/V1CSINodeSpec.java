...
public class V1CSINodeSpec {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("drivers"));

    // a set of required properties/fields (JSON key names)
    openapiRequiredFields = new HashSet<String>(Arrays.asList("drivers"));
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!V1CSINodeSpec.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in V1CSINodeSpec is not found in the empty JSON string", V1CSINodeSpec.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!V1CSINodeSpec.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `V1CSINodeSpec` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (!jsonObj.get("drivers").isJsonArray()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `drivers` to be an array in the JSON string but got `%s`", jsonObj.get("drivers").toString()));
      }
      ...
      for (int i = 0; i < jsonArraydrivers.size(); i++) {
        V1CSINodeDriver.validateJsonElement(jsonArraydrivers.get(i));
      };
  }
