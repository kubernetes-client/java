...
public class V1beta2DeviceConstraint {
   ...
   */
  @jakarta.annotation.Nullable
  @ApiModelProperty(value = "DistinctAttribute requires that all devices in question have this attribute and that its type and value are unique across those devices.  When the DRAListTypeAttributes feature gate is enabled, comparison uses set semantics (i.e., element order and duplicates are ignored): list-valued attributes must be pairwise disjoint across devices. Scalar values are treated as singleton sets for backward compatibility.  This acts as the inverse of MatchAttribute.  This constraint is used to avoid allocating multiple requests to the same device by ensuring attribute-level differentiation.  This is useful for scenarios where resource requests must be fulfilled by separate physical devices. For example, a container requests two network interfaces that must be allocated from two different physical NICs.")
   ...
   */
  @jakarta.annotation.Nullable
  @ApiModelProperty(value = "MatchAttribute requires that all devices in question have this attribute and that its type and value are the same across those devices.  For example, if you specified \"dra.example.com/numa\" (a hypothetical example!), then only devices in the same NUMA node will be chosen. A device which does not have that attribute will not be chosen. All devices should use a value of the same type for this attribute because that is part of its specification, but if one device doesn't, then it also will not be chosen.  When the DRAListTypeAttributes feature gate is enabled, comparison uses set semantics(i.e., element order and duplicates are ignored): list-valued attributes match when the intersection across all devices is non-empty. Scalar values are treated as singleton sets for backward compatibility.  Must include the domain qualifier.")
  ...
  public V1beta2DeviceConstraint requests(@jakarta.annotation.Nullable List<String> requests) {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("distinctAttribute", "matchAttribute", "requests"));
   ...
   * @throws IOException if the JSON Element is invalid with respect to V1beta2DeviceConstraint
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!V1beta2DeviceConstraint.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in V1beta2DeviceConstraint is not found in the empty JSON string", V1beta2DeviceConstraint.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!V1beta2DeviceConstraint.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `V1beta2DeviceConstraint` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if ((jsonObj.get("distinctAttribute") != null && !jsonObj.get("distinctAttribute").isJsonNull()) && !jsonObj.get("distinctAttribute").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "Expected the field `distinctAttribute` to be a primitive type in the JSON string but got `%s`", jsonObj.get("distinctAttribute").toString()));
      }
      if ((jsonObj.get("matchAttribute") != null && !jsonObj.get("matchAttribute").isJsonNull()) && !jsonObj.get("matchAttribute").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "Expected the field `matchAttribute` to be a primitive type in the JSON string but got `%s`", jsonObj.get("matchAttribute").toString()));
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!V1beta2DeviceConstraint.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'V1beta2DeviceConstraint' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<V1beta2DeviceConstraint> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(V1beta2DeviceConstraint.class));

       return (TypeAdapter<T>) new TypeAdapter<V1beta2DeviceConstraint>() {
           @Override
  ...
  /**
   * Create an instance of V1beta2DeviceConstraint given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of V1beta2DeviceConstraint
   * @throws IOException if the JSON string is invalid with respect to V1beta2DeviceConstraint
   */
  public static V1beta2DeviceConstraint fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, V1beta2DeviceConstraint.class);
  }

  /**
   * Convert an instance of V1beta2DeviceConstraint to an JSON string
   *
   * @return JSON string
   */
