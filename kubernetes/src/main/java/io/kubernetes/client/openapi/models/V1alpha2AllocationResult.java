...
public class V1alpha2AllocationResult {
  ...
  private List<V1alpha2ResourceHandle> resourceHandles = new ArrayList<>();
  ...
  * @throws IOException if the JSON Element is invalid with respect to V1alpha2AllocationResult
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!V1alpha2AllocationResult.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!V1alpha2AllocationResult.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `V1alpha2AllocationResult` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("availableOnNodes") != null && !jsonObj.get("availableOnNodes").isJsonNull()) {
        V1NodeSelector.validateJsonElement(jsonObj.get("availableOnNodes"));
      }
      if (jsonObj.get("resourceHandles") != null && !jsonObj.get("resourceHandles").isJsonNull()) {
        ...
        if (jsonArrayresourceHandles != null) {
          ...
          for (int i = 0; i < jsonArrayresourceHandles.size(); i++) {
            V1alpha2ResourceHandle.validateJsonElement(jsonArrayresourceHandles.get(i));
          };
        }
      }
  }
