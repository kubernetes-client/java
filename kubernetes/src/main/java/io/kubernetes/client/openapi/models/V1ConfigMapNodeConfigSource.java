...
public class V1ConfigMapNodeConfigSource {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("kubeletConfigKey", "name", "namespace", "resourceVersion", "uid"));

    // a set of required properties/fields (JSON key names)
    openapiRequiredFields = new HashSet<String>(Arrays.asList("kubeletConfigKey", "name", "namespace"));
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!V1ConfigMapNodeConfigSource.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in V1ConfigMapNodeConfigSource is not found in the empty JSON string", V1ConfigMapNodeConfigSource.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!V1ConfigMapNodeConfigSource.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `V1ConfigMapNodeConfigSource` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (!jsonObj.get("kubeletConfigKey").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "Expected the field `kubeletConfigKey` to be a primitive type in the JSON string but got `%s`", jsonObj.get("kubeletConfigKey").toString()));
      }
