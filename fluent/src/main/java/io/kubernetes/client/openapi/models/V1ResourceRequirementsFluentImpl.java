...
import java.util.Map;
  ...
  */
  ...
  public class V1ResourceRequirementsFluentImpl<A extends V1ResourceRequirementsFluent<A>> extends BaseFluent<A> implements V1ResourceRequirementsFluent<A>{
  ...
  public A removeFromClaims(io.kubernetes.client.openapi.models.V1ResourceClaim... items) {
    for (V1ResourceClaim item : items) {V1ResourceClaimBuilder builder = new V1ResourceClaimBuilder(item);_visitables.get("claims").remove(builder);if (this.claims != null) {this.claims.remove(builder);}} return (A)this;
  }
  ...
  public List<V1ResourceClaim> getClaims() {
    return claims != null ? build(claims) : null;
  }
  ...
  public V1ResourceClaim buildMatchingClaim(Predicate<V1ResourceClaimBuilder> predicate) {
    for (V1ResourceClaimBuilder item: claims) { if(predicate.test(item)){ return item.build();} } return null;
  }
  public Boolean hasMatchingClaim(Predicate<V1ResourceClaimBuilder> predicate) {
    for (V1ResourceClaimBuilder item: claims) { if(predicate.test(item)){ return true;} } return false;
  }
  public A withClaims(List<V1ResourceClaim> claims) {
    if (this.claims != null) { _visitables.get("claims").clear();}
    if (claims != null) {this.claims = new ArrayList(); for (V1ResourceClaim item : claims){this.addToClaims(item);}} else { this.claims = null;} return (A) this;
  }
  public A withClaims(io.kubernetes.client.openapi.models.V1ResourceClaim... claims) {
    if (this.claims != null) {this.claims.clear(); _visitables.remove("claims"); }
    if (claims != null) {for (V1ResourceClaim item :claims){ this.addToClaims(item);}} return (A) this;
  }
  public Boolean hasClaims() {
    return claims != null && !claims.isEmpty();
  }
  public V1ResourceRequirementsFluentImpl.ClaimsNested<A> addNewClaim() {
    return new V1ResourceRequirementsFluentImpl.ClaimsNestedImpl();
  }
  public V1ResourceRequirementsFluentImpl.ClaimsNested<A> addNewClaimLike(V1ResourceClaim item) {
    return new V1ResourceRequirementsFluentImpl.ClaimsNestedImpl(-1, item);
  }
  public V1ResourceRequirementsFluentImpl.ClaimsNested<A> setNewClaimLike(int index,V1ResourceClaim item) {
    return new V1ResourceRequirementsFluentImpl.ClaimsNestedImpl(index, item);
  }
  public V1ResourceRequirementsFluentImpl.ClaimsNested<A> editClaim(int index) {
  ...
  public V1ResourceRequirementsFluentImpl.ClaimsNested<A> editFirstClaim() {
  ...
  public V1ResourceRequirementsFluentImpl.ClaimsNested<A> editLastClaim() {
    int index = claims.size() - 1;
    if (index < 0) throw new RuntimeException("Can't edit last claims. The list is empty.");
    return setNewClaimLike(index, buildClaim(index));
  }
  public V1ResourceRequirementsFluentImpl.ClaimsNested<A> editMatchingClaim(Predicate<V1ResourceClaimBuilder> predicate) {
    int index = -1;
  ...
  class ClaimsNestedImpl<N> extends V1ResourceClaimFluentImpl<V1ResourceRequirementsFluentImpl.ClaimsNested<N>> implements V1ResourceRequirementsFluentImpl.ClaimsNested<N>,Nested<N>{
    ...
    int index;
    public N and() {
      return (N) V1ResourceRequirementsFluentImpl.this.setToClaims(index,builder.build());
    }
    public N endClaim() {
      return and();
    }
