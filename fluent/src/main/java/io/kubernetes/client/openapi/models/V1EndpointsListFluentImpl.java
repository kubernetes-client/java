...
import java.lang.Object;
  ...
  */
  ...
  public A removeFromItems(io.kubernetes.client.openapi.models.V1Endpoints... items) {
    for (V1Endpoints item : items) {V1EndpointsBuilder builder = new V1EndpointsBuilder(item);_visitables.get("items").remove(builder);if (this.items != null) {this.items.remove(builder);}} return (A)this;
  }
  ...
  public V1Endpoints buildMatchingItem(Predicate<V1EndpointsBuilder> predicate) {
    for (V1EndpointsBuilder item: items) { if(predicate.test(item)){ return item.build();} } return null;
  }
