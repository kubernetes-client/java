...
public class LeaderElector implements Runnable {
    ...
    /**
     * this method will block until lost the lock
     * just like RunOrDie() method in client-go
     */
    @Override
    public void run() {
        try {
    ...
    public boolean check(long maxTolerableExpiredLease) {
        if (isLeader()) {
            ...
            return System.currentTimeMillis() - observedTime > config.getLeaseDuration() + maxTolerableExpiredLease;
        }
        return false;
    }
     ...
     * return false if ctx signals done.
     */
    ...
    private boolean tryAcquireOrRenew() {
        ...
        LeaderElectionRecord leaderElectionRecord = new LeaderElectionRecord(
                config.getLock().identity(), (int) TimeUnit.MILLISECONDS.toSeconds(config.getLeaseDuration()), now, now);
