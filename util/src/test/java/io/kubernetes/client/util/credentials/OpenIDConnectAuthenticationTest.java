...
public class OpenIDConnectAuthenticationTest {
    ...
    private static final String OIDC_SERVER_KS_PATH = Resources.getResource("oidc-server.jks").getPath();
    private static final char[] OIDC_KS_PASSWORD = "changeme".toCharArray();
    private static final int PORT = 8043;

    @Rule public WireMockRule wireMockRule = new WireMockRule(WireMockConfiguration.options()
                                                                .httpsPort(PORT)
                                                                .keystoreType("JKS")
    ...
    @Test
    public void testTokenExpiredNotExpired() throws InvalidKeySpecException, NoSuchAlgorithmException, Exception {
        ...
        String jwt = TestUtils.generateJWT("someuser", "https://some.domain.nowhere",
                (PrivateKey) ks.getKey("oidc-sig", OIDC_KS_PASSWORD), TestUtils.DateOptions.Now);
    ...
    public void testTokenExpiredHasExpired() throws InvalidKeySpecException, NoSuchAlgorithmException, Exception {
        ...
        String jwt = TestUtils.generateJWT("someuser", "https://some.domain.nowhere",
                (PrivateKey) ks.getKey("oidc-sig", OIDC_KS_PASSWORD), TestUtils.DateOptions.Past);
    ...
    public void testRefreshSuccess() throws Exception {
        ...
        String refreshedJWT = TestUtils.generateJWT("someuser", "https://localhost:" + PORT,
        (PrivateKey) ks.getKey("oidc-sig", OIDC_KS_PASSWORD), TestUtils.DateOptions.Now);
        ...
        String jwt = TestUtils.generateJWT("someuser", "https://localhost:" + PORT,
                (PrivateKey) ks.getKey("oidc-sig", OIDC_KS_PASSWORD), TestUtils.DateOptions.Past);
        ...
        config.put(OpenIDConnectAuthenticator.OIDC_IDP_CERT_DATA,Base64.encodeBase64String(exportCert(
                (X509Certificate) serverKs.getCertificate("mykey")).getBytes("UTF-8")  ));
