...
class CacheTest {
  ...
  @Test
  void cachePredicateExceptionShouldExcludeObject() {
    Cache<V1Pod> podCache =
        new Cache<>(
            pod -> {
              if ("fail".equals(pod.getMetadata().getLabels().get("predicate"))) {
    ...
    podCache.add(pod);
    assertThat(podCache.list()).containsExactly(pod);

    pod.getMetadata().getLabels().put("predicate", "fail");
    podCache.update(pod);
    assertThat(podCache.list()).isEmpty();

    podCache.add(pod);
    podCache.replace(List.of(pod), "1");
